...
const currencyExchange_1 = require("../controllers/currencyExchange");
const rate = {
    currencies: {
        TWD: {
            TWD: 1,
...
describe('test with isSourceExistFunc function', () => {
    it('get source object', () => {
        ...
        expect((0, currencyExchange_1.isSourceExistFunc)(src)).toMatchObject(predictValue);
    });
...
describe('test isTargetExistFunc function', () => {
    it('get rate', () => {
        ...
        expect((0, currencyExchange_1.isTargetExistFunc)(srcObj(), target)).toMatchObject(predictValue);
    });
    it('do not get rate', () => {
        const srcCurrency = 'USD';
        const srcObj = jest.fn(() => rate.currencies[srcCurrency]);
...
describe('test amountFormatFunc function', () => {
    it('test amount normal insert', () => {
        ...
        expect((0, currencyExchange_1.amountFormatFunc)(amount)).toBe(123456);
    });
    it('test float insert', () => {
        const amount = '$123,456.1233333';
        expect((0, currencyExchange_1.amountFormatFunc)(amount)).toBe(123456.1233333);
    });
    it('test random insert', () => {
        const amount = '$rrr1t2^3';
        expect((0, currencyExchange_1.amountFormatFunc)(amount)).toBe(123);
    });
});
describe('test currencyExchangeFunc function', () => {
    it('return total amount after exchange and second decimal place', () => {
        const amount = 12;
        const rate = 30.444;
        expect((0, currencyExchange_1.currencyExchangeFunc)(amount, rate)).toBe(365.33);
    });
});
describe('test formatFinalTotal function', () => {
    it('send Int to return format type of string', () => {
        const finalTotal = 12355;
        expect((0, currencyExchange_1.formatFinalTotal)(finalTotal)).toBe('$12,355');
    });
    it('send float teturn format type of string', () => {
        const finalTotal = 12355.32;
        expect((0, currencyExchange_1.formatFinalTotal)(finalTotal)).toBe('$12,355.32');
    });
    it('send float return second decimal place with format type of string', () => {
        const finalTotal = 12355.333333;
        expect((0, currencyExchange_1.formatFinalTotal)(finalTotal)).toBe('$12,355.33');
    });
});
